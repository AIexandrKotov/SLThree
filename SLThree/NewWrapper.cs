...
namespace SLThree
{
    public static class NewWrapper
    {
        #region Type Setting
            ...
            var quickwrapper = default(Delegate);
            {
                ...
                for (var i = 0; i < elems.Count; i++)
                {
                    ...
                    if (elems[i].IsContext)
                    {
                        ...
                        wrapper_il.Emit(OpCodes.Call, typeof(NewWrapper<>).MakeGenericType(new Type[] { elem_type }).GetMethod("WrapUnder", BindingFlags.Public | BindingFlags.Static));
                    }
                    else
                    {
                    ...
                    if (!elems[i].IsStrongWrap && HasRecast(elem_type))
                        wrapper_il.Emit(OpCodes.Call, typeof(NewWrapper).GetMethod("WrapCast", BindingFlags.Public | BindingFlags.Static));
            ...
            var unwrapper = default(Delegate);
            {
                ...
                for (var i = 0; i < elems.Count; i++)
                {
                    ...
                    if (elems[i].IsContext)
                    {
                        ...
                        unwrapper_il.Emit(OpCodes.Call, typeof(NewWrapper<>).MakeGenericType(new Type[] { elem_type }).GetMethod("UnwrapUnder", BindingFlags.Public | BindingFlags.Static));
                    ...
                    else
                    {
                        ...
                        if (!elems[i].IsStrongUnwrap && HasRecast(elem_type))
                            unwrapper_il.Emit(OpCodes.Call, typeof(NewWrapper).GetMethod("UnwrapCast", BindingFlags.Public | BindingFlags.Static));
        ...
        public static ExecutionContext Wrap<T>(T obj) => NewWrapper<T>.Wrap(obj);
        public static T Unwrap<T>(ExecutionContext context) => NewWrapper<T>.Unwrap(context);
        public static void WrapIn<T>(T obj, ExecutionContext context) => NewWrapper<T>.WrapIn(obj, context);
        public static void UnwrapIn<T>(ExecutionContext context, T obj) => NewWrapper<T>.UnwrapIn(context, obj);
    }

    public static class NewWrapper<T>
    {
        ...
        static NewWrapper()
        {
            var d = NewWrapper.InitWrapper(typeof(T));
        ...
        public static ContextWrap WrapUnder(T obj, ExecutionContext context)
        {
            ...
            if (NewWrapper.HasRecast(obj.GetType())) throw new NotSupportedException("This type can't be converted into context");
            var lc = new LocalVariablesContainer(ContextSize, new Dictionary<string, int>(ContextNames));
