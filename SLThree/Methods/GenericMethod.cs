...
namespace SLThree
{
    public sealed class GenericMethod : Method
    {
        public abstract class GenericInfo
        {
            ...
            public class DictionaryCreator : GenericInfo<CreatorDictionary>
            {
                public DictionaryCreator(CreatorDictionary concrete, int position) : base(concrete, position)
                {
                }
...
/*            public class ContextCreator : GenericInfo<CreatorContextOld>
        ...
        {
            return new GenericMethod(name, ParamNames?.CloneArray(), Statements.CloneCast(), ParamTypes?.CloneArray(), ReturnType.CloneCast(), definitionplace, Implicit, Recursive, WithoutParams, DefaultValues.CloneArray(), ContantsParams.Copy(), mode, Generics.CloneArray())
            {
                Abstract = Abstract
            };
        }
